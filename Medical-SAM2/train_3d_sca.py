...
def main():
    ...
    text_layers = (
        ...
        + list(net.sam_prompt_encoder.parameters())
        + list(net.sam_mask_decoder.parameters())
        + list(net.caption_tokens.parameters())
        + list(net.sam_text_transformer.parameters())
        + list(net.language_project.parameters())
    )
    if len(sam_layers) == 0:
        optimizer1 = None
    else:
    ...
    if len(text_layers) == 0:
        ...
        optimizer3 = optim.Adam(text_layers, lr=1e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    ...
    for epoch in range(settings.EPOCH):
        ...
        caption_loss = function.train_sca(args, net, optimizer1, optimizer2, optimizer3, nice_train_loader, nice_test_loader, epoch, writer)
        ...
        if epoch % args.val_freq == 0 or epoch == settings.EPOCH-1:
            ...
            caption_loss = function.validation_sca(args, nice_test_loader, epoch, None, net, writer)
            
            # logger.info(f'Total score: {tol}, IOU: {eiou}, DICE: {edice}, caption loss: {caption_loss} || @ epoch {epoch}.')
            logger.info(f'Caption loss: {caption_loss} || @ epoch {epoch}.')
