...
def get_network(args, net, use_gpu=True, gpu_device=0, distribution=True):
    ...
    Parameters:
    - args: Arguments containing network configurations and checkpoints.
    - net: The network name ('sca2').
    - use_gpu: Boolean to specify whether to use GPU.
    - gpu_device: Device ID for the GPU (if single GPU is used).
    - distribution: Whether distributed training is being used (e.g., DDP/DeepSpeed).

    Returns:
    - net: The initialized network.
    - cfg: Configuration related to the network.
    """

    # Load the network
    if net == 'sca2':
        from sam2_train.build_sam import build_sca2_video_predictor

        sam2_checkpoint = args.sam_ckpt
        model_cfg = args.sam_config

        net, cfg = build_sca2_video_predictor(config_file=model_cfg, ckpt_path=sam2_checkpoint, mode=None)
    else:
        raise ValueError(f"Network {net} not supported")

    # If using GPU, check if distributed training is enabled
    if use_gpu:
        if distribution and torch.cuda.device_count() > 1:
            # Distributed training (e.g., DDP/DeepSpeed)
            # DDP typically handles the correct device assignment, so no need to manually move the model
            torch.cuda.set_device(gpu_device)  # Ensure the process uses the right GPU
            net = net.to(torch.device(gpu_device))
        else:
            # Single GPU case or no distributed training
            net = net.to(torch.device(gpu_device))
