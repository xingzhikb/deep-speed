...
def main(rank, world_size, args):
    ...
    if args.load_weight:
        print(f"Loading weights from {args.load_weight}")
        weights = torch.load(args.weight_path, map_location=GPUdevice)
        net.load_state_dict(weights, strict=False)
    else:
        print("Please specify the weight file to load using --load_weight")
        exit(1)
    ...
    caption_loss = function.validation_sca_ddp(
        args, nice_test_loader, epoch=0, global_step=global_step, net=net, writer=writer)
